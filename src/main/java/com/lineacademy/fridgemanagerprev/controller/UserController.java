...
@RequiredArgsConstructor      // 매개변수 생성자를 자동으로 생성해주는 어노테이션
                              // final 필드나 @NonNull 필드가 붙은 것들을 매개변수로 한
public class UserController {
    ...
    // ResponseEntity<T> : Spring-Boot Web Service에서 응답을 정의하는 제네릭 타입
    // T 자리에는 reponse.body(실제 내용이 기록되는 편지지) 의 타입이 들어가야 함
    public ResponseEntity<Map<String, Object>> createUser(
            // Spring-Boot에서는 컨트롤러의 메서드를 실행할 때,
            // 자동으로 req.body 값이 매개변수로 들어옴

            // @Valid 는 이 매개변수에 대해 검증 절차를 실행할 것이고, 실패하면 GlobalExceptionHandler로 에러를 던질 것이야
            // @RequeestBody 는 이 매개변수에 request.body 내용을 넣어줘
