package com.lineacademy.fridgemanagerprev.config;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        // 1. 헤더에서 토큰 추출
        String bearerToken = request.getHeader("Authorization");    // "Bearer fsdafjasdklfsjakflsajlflaskfjasdlfj"
        ...
        if (StringUtils.hasText(bearerToken) && bearerToken.startsWith("Bearer ")) {
            ...
            if (jwtUtil.validateToken(token)) {
                ...
                if (user != null && user.getDeletedAt() == null) {
                    ...
                    // UsernamePassowrdAuthenticationToken이라는 타입은 3칸(매개변수) 를 기록할 수 있고
                    // 첫번째 칸(매개변수)에는 사용자 ID, 두번째 칸(매개변수)에는 그 외 정보 객체, 세번째 칸(매개변수)에는 ROLE 담김
